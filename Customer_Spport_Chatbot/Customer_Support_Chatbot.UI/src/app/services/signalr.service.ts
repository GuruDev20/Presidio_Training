...
export class SignalRService {
    ...
    // signalr.service.ts
    public async leaveChat(ticketId: string, senderId: string, isAgent: boolean): Promise<void> {
        if (!ticketId) {
            throw new Error('[SignalR] Ticket ID is required');
        }
        await this.ensureConnection();
        if (this.hubConnection) {
            await this.hubConnection.invoke('LeaveChat', ticketId, senderId, isAgent)
                .catch(err => {
